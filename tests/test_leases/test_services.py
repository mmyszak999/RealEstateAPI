...
async def test_raise_exception_when_creating_lease_and_requested_property_does_not_exist(
    ...
    property_input = PropertyInputSchemaFactory().generate(
        property_status=PropertyStatusEnum.AVAILABLE, owner_id=db_staff_user.id
    )
    property_output = await create_property(async_session, property_input)

    lease_input = LeaseInputSchemaFactory().generate(
        property_id=generate_uuid(), owner_id=db_staff_user.id, tenant_id=db_user.id
    )
...
async def test_raise_exception_when_creating_lease_and_property_does_not_have_an_owner(
    ...
    lease_input = LeaseInputSchemaFactory().generate(
        property_id=property_output.id, owner_id=db_staff_user.id, tenant_id=db_user.id
    )
...
async def test_raise_exception_when_creating_lease_and_property_is_not_available(
    ...
    property_input = PropertyInputSchemaFactory().generate(
        property_status=PropertyStatusEnum.UNAVAILABLE, owner_id=db_staff_user.id
    )
    property_output = await create_property(async_session, property_input)
...
async def test_raise_exception_when_creating_lease_and_requested_owner_does_not_exist(
    ...
    lease_input = LeaseInputSchemaFactory().generate(
        property_id=property_output.id, owner_id=generate_uuid(), tenant_id=db_user.id
    )
    with pytest.raises(DoesNotExist):
        await create_lease(async_session, lease_input)
...
async def test_raise_exception_when_creating_lease_and_requested_owner_is_not_the_property_owner(
    ...
    lease_input = LeaseInputSchemaFactory().generate(
        property_id=property_output.id, owner_id=db_superuser.id, tenant_id=db_user.id
    )
...
async def test_if_only_specific_leases_were_returned(
    ...
    leases = await get_all_leases(
        async_session, PageParams(), get_with_renewal_accepted=True
    )
    assert leases.total == 1
...
async def test_check_if_lease_dates_are_updated_correctly_when_changing_lease_expiration_date(
    ...
    updated_lease = await update_single_lease(
        async_session, update_data, lease_output.id
    )
    lease_output = await get_single_lease(async_session, updated_lease.id)
    assert (
        lease_output.lease_expiration_date
        == lease_output.end_date
...
async def test_raise_exception_when_attempting_to_discard_discarded_lease_renewal(
    ...
    with pytest.raises(TenantAlreadyDiscardedRenewalException):
        await manage_lease_renewal_status(
            async_session, db_leases.results[0].id, accept_renewal=False
        )
...
async def test_leases_with_lease_expiration_date_higher_than_current_date_and_renewal_accepted_will_be_managed_correctly(
    ...
    with freeze_time(lease.lease_expiration_date + timedelta(days=1)):
        ...
        assert (
            len([lease for lease in all_leases.results if lease.lease_expired == False])
            == 1
        )
