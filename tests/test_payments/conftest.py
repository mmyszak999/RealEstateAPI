...
from src.apps.payments.services import create_payment, fulfill_payment, get_all_payments
...
async def get_stripe_session_data(payment: PaymentOutputSchema) -> dict[str, Any]:
...
async def get_payment_intent_data(lease: Lease) -> dict[str, Any]:
...
async def db_payments(
    ...
    return await get_all_payments(
        async_session, PageParams(), output_schema=PaymentOutputSchema
    )
