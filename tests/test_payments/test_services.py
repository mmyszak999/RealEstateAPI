...
from src.core.utils.orm import get_billing_period_time_span_between_payments
from src.apps.leases.services import manage_leases_with_incoming_payment_date
...
async def test_if_lease_with_payment_date_being_today_was_managed_correctly_while_creating_payment(
    ...
    with freeze_time(lease_before.next_payment_date):
        ...
        lease_after = await if_exists(Lease, "id", db_leases.results[0].id, async_session)
...
async def test_if_single_payment_was_returned(
    ...
    payment = await get_single_payment(
        async_session, db_payments.results[0].id
    )
...
async def test_specific_payments_will_returned_depending_on_the_filter_key(
    ...
    accepted_payments = await get_all_payments(async_session, PageParams(), get_accepted=True)
    assert accepted_payments.total == 1

    waiting_payments = await get_all_payments(async_session, PageParams(), get_waiting=True)
    assert waiting_payments.total == 0
    
    superuser_payments = await get_all_payments(async_session, PageParams(), tenant_id=db_superuser.id)
    assert superuser_payments.total == 1
    
    staff_user_payments = await get_all_payments(async_session, PageParams(), tenant_id=db_staff_user.id)
