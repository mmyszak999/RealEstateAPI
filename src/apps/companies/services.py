...
async def create_company(
    ...
    if company_name_check := await if_exists(
        ...
        raise AlreadyExists(
            Company.__name__, "company_name", company_data.get("company_name")
        )
...
async def get_single_company(
    ...
    company_id: str,
    output_schema: BaseModel = CompanyOutputSchema,
...
async def get_all_companies(
    ...
    output_schema: BaseModel = CompanyBasicOutputSchema,
...
async def update_single_company(
    ...
    if company_data and (
        company_data.get("company_name") != company_object.company_name
    ):
        company_name_check = await session.scalar(
            select(Company)
            .filter(Company.company_name == company_input.company_name)
            .limit(1)
        )
...
async def manage_user_company_status(
    session: AsyncSession,
    user_company_schema: UserIdSchema,
    company_id: str,
    add_user: bool = True,
) -> None:
    if not (company_object := await if_exists(Company, "id", company_id, session)):
    ...
    if not user_object.is_active:
        raise ServiceException(
            "Inactive user cannot be added or removed from the company! "
        )
...
async def add_single_user_to_company(
    ...
    return await manage_user_company_status(session, user_company_schema, company_id)
